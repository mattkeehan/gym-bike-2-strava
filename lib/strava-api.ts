...
/**
 * Upload a photo to an activity (after the activity has been created)
 * @param accessToken Valid Strava access token
 * @param activityId Activity ID to attach photo to
 * @param photoBase64 Base64-encoded photo data (with data:image/jpeg;base64, prefix)
 * @returns Upload response
 */
export async function uploadPhotoToActivity(
  accessToken: string,
  activityId: number,
  photoBase64: string
): Promise<any> {
  ...
  // Create form data
  const formData = new FormData();
  formData.append('file', photoBlob, 'workout.jpg');
  formData.append('activity_id', activityId.toString());
  formData.append('data_type', 'jpg');
  ...
  if (!response.ok) {
    const errorText = await response.text();
    console.error(`Failed to upload photo: ${response.status} ${errorText}`);
    // Don't throw - photo upload is optional
    return null;
  }
